...
class GPNet(MetaTemplate):
    ...
    def get_model_likelihood_mll(self, train_x_list=None, train_y_list=None):
        if(train_x_list is None): train_x_list=[torch.ones(100, 64)]*self.n_way
        if(train_y_list is None): train_y_list=[torch.ones(100)]*self.n_way
        model_list = list()
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x,_) in enumerate(train_loader):
            ...
            x_all = x.contiguous().view(self.n_way * (self.n_support + self.n_query), *x.size()[2:])
            y_all = Variable(torch.from_numpy(np.repeat(range(self.n_way), self.n_query+self.n_support)))
            x_support = x[:,:self.n_support,:,:,:].contiguous().view(self.n_way * (self.n_support), *x.size()[2:])
            y_support = np.repeat(range(self.n_way), self.n_support)
            x_query = x[:,self.n_support:,:,:,:].contiguous().view(self.n_way * (self.n_query), *x.size()[2:])
